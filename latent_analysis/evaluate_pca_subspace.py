"""
PCA 版感情部分空間分離の評価スクリプト。

evaluate_svm_subspace.py の PCA 版。評価ロジックは完全に同一（射影済み .pt の
"latent"/"emotion_latent"/"residual_latent" キーを読み、LinearSVC で分類するだけ）
であり、project_latents_pca.py の出力ディレクトリをそのまま渡せばよい。

    Baseline      : latent          (元の W+ コード)
    Emotion Only  : emotion_latent  (PCA 感情部分空間への射影)
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate emotion / residual subspace separation (PCA basis) via LinearSVC"
    )
