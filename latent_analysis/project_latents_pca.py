"""
PCA 版感情基底 N を使って W+ 潜在コードを感情成分と非感情成分に射影するスクリプト。

project_latents_svm.py の PCA 版。射影の数式・保存フォーマットは完全に同一であり、
本スクリプトは build_pca_projection.py が出力する emotion_basis_pca.pt を読み込むための
薄いラッパー（--basis に emotion_basis_N.pt を渡せば project_latents_svm.py でも同じ結果になる）。

射影の計算（メモリ効率版）:
    N      : (9216, k)   感情部分空間の基底（列 = ラベル相関の高い主成分）
    pinv_N : (k, 9216)   N の疑似逆行列
...
def load_basis(basis_path: str):
    """emotion_basis_pca.pt から N と pinv(N) を返す"""
    data = torch.load(basis_path, map_location='cpu', weights_only=True)
    N = data['N'].numpy()   # (9216, k)  float32
...
def project_single(w_flat: np.ndarray, N: np.ndarray, pinv_N: np.ndarray):
    ...
    """
    coords = pinv_N @ w_flat       # (k,)   感情部分空間上の座標
...
def main():
    parser = argparse.ArgumentParser(
        description="Project W+ latents into emotion / residual subspaces via PCA basis"
    )
    parser.add_argument('--basis', type=str, required=True,
                        help='Path to emotion_basis_pca.pt')
    parser.add_argument('--latent_root', type=str, default='latents',
                        help='Root directory containing split subdirectories')
    parser.add_argument('--output_root', type=str, default='latents_pca',
